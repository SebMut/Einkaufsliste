...
const exitCode = Number(process.env.IMPORT_EXIT_CODE ?? 0);
...
function liveStatusFor(source) {
  return (live.sources || []).find(s => s.store === source.store && s.market === source.market) || null;
}
function sourceTiming(source, index) {
...
const sources = (markets.sources || []).map((source, index) => {
  const liveSource = liveStatusFor(source);
  const timing = sourceTiming(source, index);
  ...
  if (!liveSource && timing.startedAt && exitCode !== 0) {
    status = 'error';
    message = 'Importer wurde während dieser Quelle beendet. Details siehe lastLogLines.';
  }
  return {
    store: source.store,
...
const diagnostics = {
  ...
  run: {
    ...
    exitCode,
    status: exitCode === 0 ? (errors ? 'partial' : 'completed') : 'failed',
  ...
  summary: {
    ...
    errors,
    skipped,
    offerCount: Array.isArray(live.offers) ? live.offers.length : 0,
...
console.log(`Diagnose geschrieben: ${ok} ok, ${noData} ohne Daten, ${errors} Fehler, ${skipped} übersprungen.`);
