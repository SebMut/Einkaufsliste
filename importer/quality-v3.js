...
const HOUSEHOLD=/waschmittel|vollwaschmittel|colorwaschmittel|feinwaschmittel|waschpulver|waschgel|waschcaps|waschpods|weichspüler|duftspüler|flecken(?:entferner|salz|spray)|spülmittel|handspülmittel|spülmaschinen|geschirrspül|klarspüler|regeneriersalz|toilettenpapier|küchenpapier|küchenrolle|taschentücher|kosmetiktücher|müllbeutel|müllsäcke|gefrierbeutel|frischhaltefolie|alufolie|backpapier|haushaltstücher|allzweckreiniger|badreiniger|wc.?reiniger|glasreiniger|küchenreiniger|bodenreiniger|entkalker|scheuermilch|reinigungstücher|schwamm|spülschwamm|bürste|geschirrtuch|seife|flüssigseife|duschgel|shampoo|zahnpasta|zahncreme|zahnbürste|zahnseide|mundspül|deo\b|deodorant|rasierer|rasiergel|rasierklinge|damenhygiene|tampon|binde\b|slipeinlage/i;
const PET=/katzenfutter|hundefutter|tierfutter|nassfutter|trockenfutter|hundesnack|katzensnack|katzenstreu|leckerl(?:i|ies)|kaustange|kauartikel/i;
const FOOD=/milch|butter|joghurt|yoghurt|käse|quark|sahne|pudding|skyr|eier|banane|apfel|äpfel|beeren|himbeer|heidelbeer|erdbeer|tomat|paprika|gurke|kartoff|zwiebel|salat|broccoli|avocado|nektarin|pfirsich|traube|gemüse|obst|mango|kiwi|melon|hack|rind|schwein|hähn|pute|wurst|salami|schinken|lachs|fisch|garnelen|steak|schnitzel|braten|fleisch|brot|brötchen|semmel|baguette|croissant|breze|nudel|pasta|reis|mehl|zucker|öl|sauce|pesto|pizza|flammkuchen|pommes|kaffee|espresso|müsli|cerealien|marmelade|konfitüre|honig|schokolade|riegel|haribo|chips|snack|keks|bonbon|eis|wasser|cola|fanta|sprite|pepsi|saft|nektar|bier|pils|wein|sekt|prosecco|whisky|gin|rum|energy|drink|nutella|frischkäse|aufschnitt|fruchtgummi|oliven|bohnen|hummus|antipasti|mayonnaise|senf|ketchup|gewürz|tee\b|cappuccino|kakao|salz\b|essig|hafer|dinkel|roggen|weizen|couscous|bulgur|linse|kichererbse|tofu|tempeh/i;
const FOOD_BRANDS=/philadelphia|ben\s*&\s*jerry|dr\.?\s*oetker|müller|landliebe|kerrygold|milram|babybel|exquisa|lätta|heinz|lindt|storck|radeberger|desperados|budweiser|barilla|funny-frisch|ritter sport|leerdammer|mövenpick|melitta|dallmayr|nescafé|haribo|kölln|zentis|iglo|greenforce|aoste|herta|bergader|beemster|berief|rotkäppchen|valdo|adelholzener|coca-cola|red bull|pringles|paulaner|nutella|alpro|bebivita|hipp|beba|cerelac|andechser|rewe bio|ja!|dmBio/i;
// Bereiche, die in dieser Einkaufs-App bewusst nicht Teil des Sortiments sind.
const NONSHOP=/steckdose|steckdosen|powertec|electric|werkzeug|akku|bohrer|schraubendreher|windschutzscheibe|elektrogerät|mehrfachstecker|verlängerungskabel|gartenmöbel|rasenmäher|grillzubehör|\bsocken\b|\bshirt\b|\bhose\b|\bjacke\b|\bschuhe\b|\bmöbel\b|\blampe\b|\bleuchte\b|matratze|\bbett\b/i;
...
const ICONS={'Baby & Kleinkind':'👶','Haushalt & Drogerie':'🧼','Tierbedarf':'🐾','Obst & Gemüse':'🥦','Milchprodukte':'🥛','Fleisch & Fisch':'🥩','Kaffee & Frühstück':'☕','Vorrat':'🍝','Tiefkühl':'🧊','Getränke':'🥤','Süßes & Snacks':'🍫','Backwaren':'🥖','Lebensmittel':'🛒'};
...
function cat(n,store,old=''){
 if(BABY.test(n))return'Baby & Kleinkind';
 if(PET.test(n))return'Tierbedarf';
 if(HOUSEHOLD.test(n))return'Haushalt & Drogerie';
 ...
 if(/nudel|pasta|reis|mehl|zucker|öl|sauce|pesto|hummus|oliven|bohnen|mayonnaise|senf|ketchup|nutella|salz\b|essig|hafer|dinkel|roggen|weizen|couscous|bulgur|linse|kichererbse/i.test(n))return'Vorrat';
...
const counts=new Map();for(const o of offers)counts.set(`${o.store}|${o.market}`,(counts.get(`${o.store}|${o.market}`)||0)+1);for(const s of data.sources||[]){const c=counts.get(`${s.store}|${s.market}`)||0;s.count=c;if(c>0){s.status='ok';s.message=`${c} streng geprüfte Produkte`;}else if(s.status==='ok'){s.status='no_data';s.message='Keine relevanten Einkaufsprodukte nach strenger Qualitätsprüfung.'}}
data.offers=offers;data.offerCount=offers.length;data.qualityV3={sanitizedAt:new Date().toISOString(),scope:'Lebensmittel + Haushalt/Drogerie + Baby/Kleinkind + Tierbedarf',before,after:offers.length,rejected:before-offers.length,repaired,reasons};report.strictFoodPass={generatedAt:new Date().toISOString(),scope:'Lebensmittel + Haushalt/Drogerie + Baby/Kleinkind + Tierbedarf',before,after:offers.length,rejected:before-offers.length,repaired,reasons,rejectedExamples:examples};await Promise.all([fs.writeFile(file,JSON.stringify(data,null,2)+'\n'),fs.writeFile(reportFile,JSON.stringify(report,null,2)+'\n')]);console.log(`Qualität V3 Einkaufsrelevanz: ${before} -> ${offers.length}; ${before-offers.length} verworfen; ${repaired} Namen bereinigt.`);
