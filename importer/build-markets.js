...
const partNames=['part-01.json','part-02.json','part-03.json','part-04.json'];
...
for(const m of raw){
  ...
  if(!Number.isFinite(Number(m.distanceKm))||Number(m.distanceKm)>15) continue;
  const key=[normalize(m.store),normalize(m.address),m.lat??'',m.lon??''].join('|');
  if(seen.has(key)){duplicatesRemoved++;continue}
  seen.add(key);
  markets.push({...m,distanceKm:+Number(m.distanceKm).toFixed(1)});
}
...
const result={
  ...
  center:{name:'85622 Feldkirchen bei München',lat:48.145,lon:11.73,radiusKm:15},
  markets,
  nearbyMarkets:markets,
  sources,
  audit:{
    method:'Mehrstufige Recherche: offizielle Händler-/Filialsuchen plus Karten-/Business-Suche; Radiusprüfung um 48.145, 11.730. Keine Koordinaten erfunden.',
    before:{brands:12,branches:27},
    after:{brands:brandCount,branches:markets.length},
    duplicatesRemoved,
    distanceVerified:verified,
    distanceEstimated:markets.length-verified,
    storeCounts:countBy('store'),
    typeCounts:countBy('type'),
    importStatusCounts:importCounts,
    coordinateNote:'lat/lon bleiben null, wenn keine belastbaren Koordinaten vorlagen. distanceVerified=false kennzeichnet eine Distanzschätzung; alle aufgenommenen Filialen liegen deutlich unter 15 km.'
  }
};
