...
$('page').each((pi,pel)=>{const page={page:pi+1,width:Number($(pel).attr('width')),height:Number($(pel).attr('height')),priceBlocks:[]};const blocks=[];$(pel).find('block').each((bi,bel)=>{const words=[];$(bel).find('word').each((_,w)=>words.push({t:$(w).text(),x1:Number($(w).attr('xMin')),y1:Number($(w).attr('yMin')),x2:Number($(w).attr('xMax')),y2:Number($(w).attr('yMax'))}));if(!words.length)return;blocks.push({i:bi,x1:Math.min(...words.map(w=>w.x1)),y1:Math.min(...words.map(w=>w.y1)),x2:Math.max(...words.map(w=>w.x2)),y2:Math.max(...words.map(w=>w.y2)),text:words.map(w=>w.t).join(' '),words});});
 for(const block of blocks){if(!/\d{1,3}[,.]\d{2}\*?\s*\(\d{1,3}[,.]\d{2}\)/.test(block.text))continue;
   const left=Math.max(0,block.x1-230),right=Math.min(page.width,block.x2+25),top=Math.max(0,block.y1-220),bottom=block.y2+20;
   const near=blocks.filter(b=>b.i!==block.i&&b.y2>=top&&b.y1<=bottom&&b.x2>=left&&b.x1<=right)
     .sort((a,b)=>a.y1-b.y1||a.x1-b.x1).slice(0,40)
     .map(b=>({i:b.i,x1:+b.x1.toFixed(1),y1:+b.y1.toFixed(1),x2:+b.x2.toFixed(1),y2:+b.y2.toFixed(1),text:b.text.slice(0,350)}));
   page.priceBlocks.push({block:{i:block.i,x1:+block.x1.toFixed(1),y1:+block.y1.toFixed(1),x2:+block.x2.toFixed(1),y2:+block.y2.toFixed(1),text:block.text.slice(0,500)},window:{left:+left.toFixed(1),right:+right.toFixed(1),top:+top.toFixed(1),bottom:+bottom.toFixed(1)},near});if(page.priceBlocks.length>=20)break;}pages.push(page)});
