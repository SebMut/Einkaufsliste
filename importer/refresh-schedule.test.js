...
import {berlinParts,shouldRunScheduled,nextScheduledRun,previousScheduledRun,TARGET_HOURS} from './refresh-schedule.js';
...
test('Zwischenstunden werden nicht ausgeführt',()=>{
  assert.equal(shouldRunScheduled(new Date('2026-07-01T05:00:00Z')),false);
...
test('Recovery erkennt den letzten fälligen Soll-Lauf',()=>{
  const previous=previousScheduledRun(new Date('2026-08-20T16:35:00Z'));
  const p=berlinParts(previous);
